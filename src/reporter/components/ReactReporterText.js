...
class ReactReporterText extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        return nextState.mode !== this.state.mode
            ...
            || nextProps.analizerFunction !== this.props.analizerFunction;
    }
    ...
    handleToolboxAnalizer(ev) {
        ev.stopPropagation();
        if (this.props.analizer && this.props.analizer.function) {
            this.analizerArr = this.props.analizer.function();
            this.opacity = 1; /*0*/
            this.text = '';
            this.analizerArr.forEach(obj => this.text += obj.string);
            this.setState({
                mode: 'read',
                text: this.text
            })
    ...
    render() {
        ...
        const textToolbox = () => {
            return <div className="react-reporter-text-toolbox"
                ...
                <div className="react-reporter-text-toolbox-item react-reporter-text-toolbox-analisis" title="get text from data analysis" onClick={this.handleToolboxAnalizer.bind(this)}>
        ...
        const analizerElement = () => {
            ...
            if (this.props.analizer && this.props.analizer.type && this.analizerArr) {
                switch(this.props.analizer.type) {
                    case 'map':
                        let idMap = id();

                        // style={{fontWeight: 'bold'}} needed for Firefox
                        element = this.analizerArr.map((item, index) => {
                            return <div id={"analizer-text-" + index + '-' + idMap} style={{opacity: this.opacity, /*transition: (index + 0.5) + 's'*/}}
                                        key={index}>{item.parts[0]}<b style={{fontWeight: 'bold'}}>{item.parts[1]}</b>{item.parts[2]}
                                        <b style={{fontWeight: 'bold'}}>{item.parts[3]}</b><br/><br/></div>;
                        });

                        /*setTimeout(() => {
                            for (let i = 0; i < this.analizerArr.length; i++) {
                                document.getElementById("analizer-text-" + i  + '-' + idMap).style.opacity = 1;
                            }
                        }, this.timeout);*/
                        break;
                    case 'table':
                        ...
                        element = this.analizerArr.map((item, index) => {
                            return <div id={"analizer-text-" + index  + '-' + idTable} style={{opacity: this.opacity, /*transition: (index + 0.5) + 's'*/}}
                                        key={index}>{item.parts[0]}<b style={{fontWeight: 'bold'}}>{item.parts[1]}</b><br/><br/></div>;
                        });

                        /*setTimeout(() => {
                            for (let i = 0; i < this.analizerArr.length; i++) {
                                document.getElementById("analizer-text-" + i  + '-' + idTable).style.opacity = 1;
                            }
                        }, this.timeout);*/
                        break;
                    case 'trend':
                        ...
                        // style={{fontWeight: 'bold'}} needed for Firefox
                        element = this.analizerArr.map((item, index) => {
                            return <div id={"analizer-text-" + index  + '-' + idTrend} style={{opacity: this.opacity, /*transition: (index + 0.5) + 's'*/}}
                                        key={index}><b style={{fontWeight: 'bold'}}>{item.parts[0]}</b>{item.parts[1]}{item.parts[2]}
                                        <br/><br/></div>;
                        });

                        /*setTimeout(() => {
                           for (let i = 0; i < this.analizerArr.length; i++) {
                               document.getElementById("analizer-text-" + i  + '-' + idTrend).style.opacity = 1;
                           }
                        }, this.timeout);*/
                }
            }
        ...
        const textElement = () => {
          ...
          if (this.props.text && this.props.text !== '') {
              ...
              if (this.state.mode === 'read') {
                  // Clear analizerArr after first rendering to be able to add text from textarea
                  // But only in case if it's text not equal state text in order not to loose formatting
                  // Till WYSIWYG editor will be added
                  if (this.analizerArr) {
                      let analizerText = '';
                      this.analizerArr.forEach(obj => analizerText += obj.string);
                      if (analizerText !== this.state.text) {
                          this.analizerArr = undefined;
                      }
                  }

                  let text = (this.props.analizer && this.analizerArr) ? analizerElement() : this.state.text;

                  textEl = <div className={"react-reporter-text-read-mode"  + ' ' +  externalStyle}>{text}</div>
              } else if (this.state.mode === 'write') {
